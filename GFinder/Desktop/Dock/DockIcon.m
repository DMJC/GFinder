  ...
  if ([theEvent clickCount] >= minimumLaunchClicks) {
    if ([self isSpecialIcon] == NO) {
      if (launched == NO) {
        ...
        [[GFinder gfinder] unhideAppWithPath: [node path] andName: appName];
      } else {
        [[GFinder gfinder] activateAppWithPath: [node path] andName: appName];
      }
    } else if (isWsIcon) {
...
- (NSMenu *)menuForEvent:(NSEvent *)theEvent
{
  if ([self isSpecialIcon] == NO) {
    ...
    if (appPath) {
      ...
      app = [[GFinder gfinder] launchedAppWithPath: appPath
                                                 andName: appName];      
