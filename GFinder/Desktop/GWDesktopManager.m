...
{
  [gfinder addWatcherForPath: path];
}

- (void)removeWatcherForPath:(NSString *)path
{
  [gfinder removeWatcherForPath: path];
}

- (void)showRootViewer
{
  [gfinder newViewerAtPath: path_separator()];
}

- (BOOL)selectFile:(NSString *)fullPath
inFileViewerRootedAtPath:(NSString *)rootFullpath
{
  return [gfinder selectFile: fullPath inFileViewerRootedAtPath: rootFullpath];
}

- (void)performFileOperation:(NSDictionary *)opinfo
{
  [gfinder performFileOperation: opinfo];
}
                      
- (NSString *)trashPath
{
  return [gfinder trashPath];
}
    ...
    if ([node hasValidPath]) {           
      ...
      if ([node isDirectory]) {
        if ([node isPackage]) {    
          ...
          [gfinder newViewerAtPath: [node path]];
        } 
...
{
  [gfinder openSelectedPathsWith];
}

- (void)newFolder
{
  [gfinder newObjectAtPath: [dskNode path] isDirectory: YES];
}

- (void)newFile
{
  [gfinder newObjectAtPath: [dskNode path] isDirectory: NO];
}
...
- (void)showTerminal
{
  [gfinder startXTermOnDirectory: [dskNode path]];
}
