  ...
  if (self)
    {
      ...
      ASSIGN (bviewerHelp, [gfinder contextHelpFromName: @"BViewer.rtfd"]);
      
      viewers = [NSMutableArray new];
      orderingViewers = NO;
      
      historyWindow = [gfinder historyWindow]; 
  ...
  for (i = 0; i < [vwrs count]; i++)
    {
      ...
      for (j = 0; j < [watchedNodes count]; j++)
        [gfinder removeWatcherForPath: [[watchedNodes objectAtIndex: j] path]];
    }
...
- (void)selectionChanged:(NSArray *)selection
{
  if (orderingViewers == NO) {
    [gfinder selectionChanged: selection];
  }
}
  ...
  if ([node isValid] && (settingHistoryPath == NO))
    {
      ...
      int cachemax = [gfinder maxHistoryCache];
      int count;
