...
@implementation GFinder (WorkspaceApplication)
                         ...
                         tag:(NSInteger *)tag
{
  if (loggingout == NO)
    ...
    {
      NSRunAlertPanel(nil, 
		      NSLocalizedString(@"GFinder is logging out!", @""),
...
@implementation GFinder (Applications)
  ...
  NS_HANDLER
    {
      ...
      NSLog(@"GFinder caught exception %@: %@", 
...
{  
  if (application) {
    ...
    NS_HANDLER
      {
    GWDebugLog(@"GFinder caught exception %@: %@", 
	                      [localException name], [localException reason]);
      }
    NS_ENDHANDLER
  } else { 
