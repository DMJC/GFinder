...
- (void)startOperation
{
  if (confirm)
    {    
      ...
      else if ([type isEqual: @"GFinderRecycleOutOperation"])
        {
          title = NSLocalizedString(@"Recycler", @"");
          msg1 = [NSString stringWithFormat: @"%@ %@ %@ ", 
                           NSLocalizedString(@"Move", @""), 
      ...
      else if ([type isEqual: @"GFinderEmptyRecyclerOperation"])
        {
          title = NSLocalizedString(@"Recycler", @"");
...
- (oneway void)performOperation
{
  ...
  else if ([operation isEqual: @"GFinderRenameOperation"])
    {
      [self doRename];
    }
  else if ([operation isEqual: @"GFinderCreateDirOperation"])
    {
      [self doNewFolder];
    }
  else if ([operation isEqual: @"GFinderCreateFileOperation"])
    {
      [self doNewFile];
    }
}
